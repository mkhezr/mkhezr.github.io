...
# Download work data from google sheets.
works_sheet <- gs_title("works")

if(works_sheet$updated > file.mtime("work.csv")){
  #updating work.csv
  works_sheet %>%
    gs_download(ws = "work", to = "work.csv", overwrite = TRUE)   
}

if(works_sheet$updated > file.mtime("tag.csv")){
  # updating tag.csv
  works_sheet %>%
    gs_download(ws = "tag", to = "tag.csv", overwrite = TRUE)  
} 
